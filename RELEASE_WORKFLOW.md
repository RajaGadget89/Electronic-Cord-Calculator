...
## หลักการฟีเจอร์ที่ "บันทึกงานเพื่อดูย้อนหลัง" (ต้องเหมือนกันทุกฟีเจอร์)
ทุกฟีเจอร์ที่บันทึกงานได้ (ออกแบบ, ตรวจสอบ, และฟีเจอร์ในอนาคต) ต้องทำงานแบบเดียวกัน:
1. มีช่อง **ชื่องาน + แท็ก** และปุ่ม **บันทึก**
2. บันทึกลง IndexedDB โดยใช้ `StoredJob.kind` แยกประเภท (พร้อม accessor ที่เข้ากับงานเก่า)
3. หน้าแรกแสดง **ป้ายกำกับประเภทงาน** + กรองตามประเภทได้
4. เปิดงานย้อนหลังแล้ว **แสดงค่าที่บันทึกไว้ครบทันที + ผลลัพธ์ + ปุ่มรายงานทั้งหมด** (คัดลอก/Markdown/พิมพ์/PDF/แชร์) + ลิงก์กลับไปแก้ไข — ห้ามให้ผู้ใช้ต้องกรอกซ้ำ
5. รองรับ **ทำซ้ำ (duplicate)** ทั้งสองประเภท
> หมายเหตุ input ที่มี state ภายใน (เช่น `CurrentField`) ต้องรับค่าเริ่มต้น (เช่น `initialAmps`) เพื่อ prefill ตอนเปิดย้อนหลัง
